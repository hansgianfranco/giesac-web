...
document.getElementById('filtros').innerHTML=categorias.map((c,i)=>
  `<button class="filtro ${i===0?'active':''}" data-f="${c.key}">${c.label}</button>`).join('');
...
function renderProyectos(filter){
  ...
  grid.innerHTML=lista.map((p,idx)=>{
    ...
    return `
      <div class="proyecto" data-cat="${p.cat}">
        <img src="${img}" alt="${p.nombre}" loading="lazy">
        <span class="num-tag">#${p.n}</span>
        <span class="cliente-tag">${p.cliente!=='—'?p.cliente:'Ref.'}</span>
        <div class="overlay"></div>
        <div class="proyecto-info">
          <h4>${p.nombre}</h4>
          <div class="loc">${p.loc}${p.area!=='—'?' · '+p.area:''}</div>
          <div class="ficha">${p.desc}</div>
        </div>
      </div>`;
  }).join('');
}
